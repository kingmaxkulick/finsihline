...
for dbc_path in DBC_FILE_PATHS:
    try:
        ...
        print(f"[SUCCESS] Loaded DBC file: {dbc_path}")
    except Exception as e:
        print(f"[ERROR] Error loading DBC file {dbc_path}: {e}")

# Get all valid CAN IDs from the merged database
valid_can_ids = {msg.frame_id: msg.name for msg in dbc.messages}
print(f"[SUCCESS] Available CAN Messages: {valid_can_ids}")
...
log_start_time = None
logging_thread = None
log_interval = 0.05  # 5 milliseconds
current_log_id = 0
log_directory = "./logs"
signals_to_log = None
...
def receive_can_data():
    ...
    try:
        ...
        print("[SUCCESS] Connected to PEAK CAN interface")
                ...
                if message:
                    ...
                    if message.arbitration_id in valid_can_ids:
                        ...
                        try:
                            ...
                            print(f"[WARNING] Error decoding message {message_name} (ID: 0x{message.arbitration_id:X}): {decode_error}")
            
            except can.CanError as e:
                print(f"[WARNING] CAN Bus error: {e}")
                time.sleep(1)  # Wait a bit before retrying
                
    except Exception as setup_error:
        print(f"[ERROR] Failed to setup CAN interface: {setup_error}")
...
def logging_thread_function():
    ...
    print(f"[INFO] Starting high-frequency logging thread (interval: {log_interval*1000}ms)")
    log_count = 0
...
async def upload_dbc(file: UploadFile = File(...)):
    ...
    try:
        ...
        dbc = new_dbc
        valid_can_ids = {msg.frame_id: msg.name for msg in dbc.messages}
        print(f"[SUCCESS] New DBC Loaded: {file.filename}")
...
async def start_logging(request: LoggingRequest):
    ...
    print(f"[SUCCESS] Started logging with ID {current_log_id} (interval: {log_interval*1000}ms)")
    if signals_to_log:
...
async def stop_logging(background_tasks: BackgroundTasks):
    ...
    print(f"[SUCCESS] Stopped logging. Saving to {filename} ({len(log_data)} entries)")
    print(f"[INFO] Cleaning up old log files (keeping most recent 5)")
    return {
        "status": "stopped", 
...
def save_log_to_csv(data: List[Dict[str, Any]], filepath: str):
    ...
    try:
        ...
        print(f"[SUCCESS] Log file saved successfully: {filepath}")
        return True
    except Exception as e:
        ...
        if len(log_files) > max_files_to_keep:
            ...
            for file_info in files_to_delete:
                try:
                    ...
                    print(f"[INFO] Deleted old log file: {file_info['filename']}")
                except Exception as e:
                    print(f"[WARNING] Error deleting {file_info['filename']}: {e}")
...
def shutdown_event():
    ...
    print("[INFO] Shutting down CAN receiver")
    
    # Stop logging if active
    if is_logging:
        print("[WARNING] Logging was active during shutdown, attempting to save data...")
        stop_logging_event.set()
